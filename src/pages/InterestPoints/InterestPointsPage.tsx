...
export default function InterestPointsPage() {
    ...
    const createMut = useMutation({
        mutationFn: (values: FormValues) => createInterestPoint(values),
        onSuccess: () => qc.invalidateQueries({ queryKey: ['interest-points'] }),
    });

    const updateMut = useMutation({
        mutationFn: (payload: { id: number | string; values: Partial<InterestPoint> }) =>
            updateInterestPoint(payload.id, payload.values),
        onSuccess: () => qc.invalidateQueries({ queryKey: ['interest-points'] }),
    });
        ...
        <Stack spacing={2}>
            <Stack direction="row" justifyContent="space-between" alignItems="center">
                ...
                <Stack direction="row" spacing={1}>
                    <TextField size="small" placeholder="Buscar…" value={q} onChange={(e) => { setQ(e.target.value); setPage(1); }} />
                    ...
                    <Button variant="contained" onClick={() => setOpen({ mode: 'create' })}>Nuevo POI</Button>
                </Stack>
            </Stack>

            {isError && <Alert severity="error">{(error as any)?.response?.data?.message ?? 'Error al cargar'}</Alert>}
                ...
                <Table size="small">
                    ...
                    <TableBody>
                        {isLoading ? (
                            ...
                            <TableRow><TableCell colSpan={5}>Sin resultados</TableCell></TableRow>
                        ) : (
                            rows.map((r) => {
                                    ...
                                    <TableRow key={r.id} hover>
                                        ...
                                        <TableCell align="right">
                                            <Tooltip title="Geocodificar (lat/lng)">
                                                <span>
                                                    <IconButton size="small" onClick={() => geocodeMut.mutate(r.id)} disabled={geocodeMut.isPending}>
                                                        <MyLocationIcon fontSize="small" />
                                                    </IconButton>
                                                </span>
                                            </Tooltip>
                                            <Tooltip title={active ? 'Desactivar' : 'Activar'}>
                                                <Switch size="small" checked={active} onChange={() => statusMut.mutate({ id: r.id, next: !active })} />
                                            </Tooltip>
                                            ...
                                            <IconButton size="small" title="Imágenes" onClick={() => setOpenImagesForPoi(Number(r.id))}>
            ...
            <PoiDialog
                ...
                defaultValues={open && 'row' in open ? {
                    name: open.row.name,
                    city: open.row.city ?? '',
                    address: open.row.address ?? '',
                    latitude: open.row.latitude ?? undefined,
                    longitude: open.row.longitude ?? undefined,
                    status: open.row.status,
                } : { name: '', city: '', address: '', latitude: undefined, longitude: undefined, status: true }}
                    ...
                    <Button color="error"
                        onClick={async () => {
                            if (confirmDelete) {
                                ...
                                } catch (_) { /* manejar popup/toast si lo agregas */ }
                            }
                        }}
                    >Eliminar</Button>
                </DialogActions>
            ...
            <DialogContent sx={{ pt: 2 }}>
                ...
                <Stack spacing={2}>
                    ...
                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                        <TextField label="Ciudad" size="small" {...register('city')} />
                        <TextField label="Dirección" size="small" fullWidth {...register('address')} />
                    </Stack>
                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                        <TextField label="Latitud" size="small" type="number" inputProps={{ step: 'any' }} {...register('latitude', { valueAsNumber: true })} />
                        <TextField label="Longitud" size="small" type="number" inputProps={{ step: 'any' }} {...register('longitude', { valueAsNumber: true })} />
                    </Stack>
                    <FormControl size="small">
                        ...
                        <Controller
                            ...
                            control={control}
                            render={({ field }) => (
                                <Select label="Estado" value={field.value} onChange={(e) => field.onChange(e.target.value)}>
